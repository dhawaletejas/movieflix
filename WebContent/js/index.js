...
myApp.controller("Ctrl", function($scope, $filter) {
    $scope.addHover = function(n) {
        if(n>0 && n<5) {
            angular.element(document).find('.row-item-1').addClass('hoverEffectShiftRight');
        }
        else {
            angular.element(document).find('.row-item-1').removeClass('hoverEffectShiftRight');
        }
        if(n==5) {
            angular.element(document).find('.row-item-1').addClass('hoverEffectShiftLeft');
        }
        else {
            angular.element(document).find('.row-item-1').removeClass('hoverEffectShiftLeft');
        }
    }
    $scope.items =
        [
    ...
    $scope.movieData = [];
    for(var i=0;i<$scope.items.length/6;i++) {
            $scope.movieData.push({id:i , child:$scope.items.splice(0,6)})
    }
});
